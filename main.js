document.addEventListener("DOMContentLoaded", function () {
  ...
  function toggleMainSections(showMain) {
    var mainSections = document.querySelectorAll(
      ".hero, .shop-section, .about-section, .reviews-section, .contact-section"
    );
  ...
  function showProductDetail(card) {
    ...
    if (relatedGrid) {
      ...
      products.forEach(function (other) {
        ...
        // If current product has unisex, show all products that have unisex
        if (currentHasUnisex) {
          showProduct = otherHasUnisex;
        } else if (currentHasMen) {
          // If current is men, show men and unisex products
          showProduct = otherHasMen || otherHasUnisex;
        } else if (currentHasWomen) {
          // If current is women, show women and unisex products
          showProduct = otherHasWomen || otherHasUnisex;
        }
        ...
        clone.addEventListener("click", function (e) {
          if (e.target.closest(".size-option") || e.target.closest(".btn.btn-outline")) return;
          showProductDetail(other);
        });
        
        // Add click listener for the order button in cloned cards
        var cloneOrderButton = clone.querySelector(".product-footer .btn.btn-outline");
        if (cloneOrderButton) {
          cloneOrderButton.addEventListener("click", function (e) {
            e.stopPropagation();
            
            var cloneNameEl = clone.querySelector(".product-name");
            var cloneNameText = cloneNameEl ? cloneNameEl.textContent.trim() : "";
            
            var cloneActiveSizeBtn = clone.querySelector(".size-option.is-active");
            var cloneSizeText = cloneActiveSizeBtn ? cloneActiveSizeBtn.getAttribute("data-size") + "ml" : "125ml";
            
            var cloneWhatsappText = "أرغب في طلب منتج " + cloneNameText + " - الحجم: " + cloneSizeText + " من MALORA Body Splash";
            var cloneUrl = WHATSAPP_BASE + encodeURIComponent(cloneWhatsappText);
            
            window.open(cloneUrl, "_blank");
          });
        }
        
        // Add click listeners for size options in cloned cards
        var cloneSizeButtons = Array.prototype.slice.call(clone.querySelectorAll(".size-option"));
        cloneSizeButtons.forEach(function (btn) {
          btn.addEventListener("click", function (e) {
            e.stopPropagation();
            cloneSizeButtons.forEach(function (b) {
              b.classList.remove("is-active");
            });
            btn.classList.add("is-active");
            ...
            var cloneSizeEl = clone.querySelector(".product-size");
            if (cloneSizeEl && size) {
              cloneSizeEl.textContent = size + "ml";
            }
            
            var clonePriceEl = clone.querySelector(".product-price");
            if (clonePriceEl && price) {
              clonePriceEl.textContent = price + " EGP";
            }
          });
        });
